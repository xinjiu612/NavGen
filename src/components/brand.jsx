// ModelScope — Simple Icons (CC0), inlined so the page needs no icon CDN.
export function ModelScopeIcon({ size = 17, ...rest }) {
  return (
    <svg
      viewBox="0 0 24 24"
      width={size}
...
// Hugging Face — Simple Icons (CC0), inlined so the page needs no icon CDN.
export function HuggingFaceIcon({ size = 17, ...rest }) {
  return (
    <svg
      viewBox="0 0 24 24"
      width={size}
...
// arXiv — Simple Icons (CC0), inlined so the page needs no icon CDN.
export function ArXivIcon({ size = 17, ...rest }) {
  return (
    <svg
      viewBox="0 0 24 24"
      width={size}
...
/**
 * The dataset / paper badges, shared by the hero and the citation section.
 * An entry with `href: null` renders as a dimmed "soon" chip instead of a
 * link, so a placeholder never ships as a dead URL.
 */
export function BrandLinks({ links, className = '' }) {
  return (
      ...
      {links.map(({ key, href, label, Icon }) =>
        href ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.04] px-3.5 py-2 text-[0.8rem] text-txt-dim transition hover:border-cyan/40 hover:bg-white/[0.08] hover:text-txt"
          >
            <Icon className="shrink-0 text-txt-mute transition group-hover:text-cyan" />
            {label}
          </a>
        ) : (
          <span
            ...
            className="inline-flex cursor-default items-center gap-2 rounded-full border border-white/[0.07] bg-white/[0.015] px-3.5 py-2 text-[0.8rem] text-txt-mute"
          >
            <Icon className="shrink-0 opacity-50" />
            {label}
            <span className="mono text-[0.6rem] uppercase tracking-wider opacity-60">soon</span>
          </span>
        ),
      )}
    </div>
